...
2. How many of the recorded yarns are machine washable?
    ...
    > My code before the first pipe tells the system that we are looking at the yarn dataset, in the next pipe it filters out all N/A values in the machine_washable variable, finally it filters out all responses of false in the machine_washable variable 
    
    > The original data type is boolean (true/false). then I take the sum it R or puthon converts booleans to on interger 1/0 respectively. NA values cannot be added in either language, so I first remove then replace the NA values with false) and then take the sum of the converted intergers. The result is stored in n_washable
...
MachineWashable<- yarn |> filter(!is.na(machine_washable)) |> filter(machine_washable == TRUE)

count(MachineWashable)
...
n_washable = yarn.fillna(False).machine_washable.sum()
```
...
NAval<- length(is.na(yarn[1]))

NAval<- yarn
NAvals<- c()
for(i in 1:24) {
  NAval[i]<- is.na(yarn[i])
  NAvals<- c(NAvals, sum(NAval[i]==TRUE))
}

NAvals

for(i in 1:ncol(yarn)) {
  missing_vals <- sum(is.na(yarn[,i]))
}
...
names(yarn[which.max(NAvals)])
names(yarn[which.min(NAvals)])
median(NAvals)
```
...
4. Consider the column `yarn_company_name`.  
    ...
    the `yarn` data is in tidy form: there is a column for each variable, a row for each observation, and a cell that contains only one value. The `common_yarn` data is similar - it has only one ID variable (yarn_company_name) and one calculated variable, `n`, but this is still tidy.
...
5. Consider the columns `texture` and `texture_clean`. 
    ...
    The codebook and cleaning script say that `stringr` `str_trim` and `str_to_lower` were used to create `texture_clean`.
    ...
    Seperate everything into single-phrase (split by commas),
    Assemble into a big list (unlist in R)
    count # occurences
...
yarn <- yarn |> mutate(common_texture = is_top_descriptor(texture_clean))
...
all_descriptors = yarntmp.texture_clean.str.split('[,:]', regex= True).explode()
...
6. Consider the variable `yarn_weight_wpi`.
    ...
    > 1. 0-4 > 4 and 5-6 > 6
    > 2. convert string values to intergers NA will stay NA
    > 3. 
...
# convert_weight_int(c("8", "0-4", "5-6", NA)) 
...
yarn_no_na <- yarn |> filter(!is.na(yarn_weight_wpi_num))
